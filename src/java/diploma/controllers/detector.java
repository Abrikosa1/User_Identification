...
public class detector {
    ...
    public static void detect(JSONArray jsonArr,String login, int id) throws IOException {  
        for(int i=0;i<jsonArr.length();i++){
        String strIm = jsonArr.get(i).toString();
        BufferedImage buffImg = detector.String2BufferedImage(strIm);
        //System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
        Mat matImg = BufferedImage2Mat(buffImg);
        //upint x = 0, y = 0, height = 0, width = 0;
        Rect rectCrop = null;
...
//        }
        ...
        int count = i+1;
        Imgcodecs.imwrite("C:\\trainingImages2\\"+id+"-"+login +"-"+ count + ".png", fin);
        }
    }
